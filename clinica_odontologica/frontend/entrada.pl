...
tabelas_clinica -->
    html(div(class('container-fluid py-3'),
             [ \cabeça_da_tabela('Tabela Funcionario', '/funcionario'),
               \tabela,
               \cabeça_da_tabela('Tabela Agenda', '/agenda'),
               \tabela
             ]
            )).
...
/*Conferir concatenação de atributos com variável Tabela*/
ações(Id, Campo, Tabela):-
    Campo = [ a([ class('text-success'), title('Alterar'),
                  href('/', Tabela, '/editar/~w' - Id),
                  'data-toggle'(tooltip)],
                [ \lápis ]),
              a([ class('text-danger ms-1'), title('Excluir'),
                  href('/api/v1/', Tabela, '/~w' - Id),
